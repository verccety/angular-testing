...
describe("HomeComponent", () => {
  ...
  it("should display advanced courses when tab clicked", (done: DoneFn) => {
    // With `done: DoneFn` Jasmine will not consider test finished until it find done() call
  ...
  it("should display advanced courses when tab clicked", fakeAsync(() => {
    ...
    // или tick(16) если знаем внутреннюю структуру, в данном случае вызывается requestAnimationFrame, он обновляется каждые 16мс обычно
    const cardTitles = el.queryAll(
      By.css(".mat-mdc-tab-body-active .mat-mdc-card-title")
    );
  ...
  it("should display advanced courses when tab clicked - waitForAsync", waitForAsync(() => {
    // waitForAsync поддерживает http запросы в отличие от fakeAsync
